...
- **Kategorisierte Paketlisten** – 11 Kategorien, ~389 Pakete
...
├── scripts/            15 Helper-Scripts → symlinked nach ~/.local/bin/
...
| `desktop.txt`     | waybar, wofi, dunst, hyprlock, swww         |
| `theming.txt`     | qt5ct, qt6ct, kvantum, nwg-look             |
| `media.txt`       | PipeWire-Stack, imagemagick, grim, slurp    |
...
| `aur.txt`         | bibata-cursor, candy-icons, p10k-git        |
| `optional.txt`    | discord, steam, obs, gimp, libreoffice      |
...
- **hypr/** – Hyprland + 6 Subconfigs + hypridle.conf
- **waybar/** – Statusbar
- **kitty/** – Terminal
- **wofi/** – App-Launcher
...
Scripts aus `scripts/` werden nach `~/.local/bin/` verlinkt.
